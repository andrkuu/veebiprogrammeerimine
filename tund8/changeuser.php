...
//kui on andmete muutmise nuppu vajutatud
  if (isset($_POST["submitUserData"])) {
      ...
      if (!isset($_POST["currentpassword"]) or empty($_POST["currentpassword"])) {
          $passwordError = "Palun sisesta salasõna!";
      } else {
          if (strlen($_POST["currentpassword"]) < 8) {
              $passwordError = "Liiga lühike salasõna (sisestasite ainult " . strlen($_POST["currentpassword"]) . " märki).";
          }
      }
      ...
      if (empty($nameError) and empty($surnameError) and empty($birthMonthError) and empty($birthYearError) and empty($birthDayError) and empty($birthDateError) and empty($genderError) and empty($emailError) and empty($passwordError)) {

          $options = ["cost" => 12, "salt" => substr(sha1("rBffUdkxZ"), 0, 22)];
          $pwdhash = password_hash($_POST["currentpassword"], PASSWORD_BCRYPT, $options);

          if (isset($_POST["newpassword"]) or !empty($_POST["newpassword"])) {
              if ($userData["password"] == $pwdhash){

                  $notice = changeUserData($name, $surname, $email, $gender, $birthDate, $_POST["newpassword"]);
              }
              else{
                  $notice = "Vana parool ei olnud õige";
              }
          }
          else{
              if ($userData["password"] == $pwdhash){

                  $notice = changeUserData($name, $surname, $email, $gender, $birthDate, $_POST["currentpassword"]);
              }
              else{
                  $notice = "Vana parool ei olnud õige";
              }
...
<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
    ...
    <label><input type="radio" name="gender" value="2" <?php if ($gender == "2") {
            echo " checked";
        } ?>>Naine</label>
    <label><input type="radio" name="gender" value="1" <?php if ($gender == "1") {
            echo " checked";
        } ?>>Mees</label><br>
    ...
    <input name="currentpassword" type="password"><span><?php echo $passwordError; ?></span><br>
    <label>Uus salasõna:</label><br>
    <input name="newpassword" type="password"><span><?php echo $confirmpasswordError; ?></span><br>
    <input name="submitUserData" type="submit" value="Muuda andmeid"><span><?php echo $notice; ?></span>
</form>
<hr>
