...
function readUserData($userId){
    ...
    $stmt = $conn->prepare("SELECT firstname,lastname,birthdate,gender,email,created,password FROM vpusers WHERE id=?");
    echo $conn->error;
    $stmt->bind_param("s", $userId);
    //$stmt->bind_result($firstname,$lastname, $birthdate,$gender,$email,$created);
...
function changeUserData($name, $surname, $email, $gender, $birthDate, $password){
    $notice = "";
    $conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $conn->prepare("SELECT id FROM vpuserprofiles WHERE id=?");
    echo $conn->error;
    $stmt->bind_param("i", $_SESSION["userId"]);
    $stmt->bind_result($idFromDb);
    $stmt->execute();
    ...
    if($stmt->fetch()){
        ...
        $stmt = $conn->prepare("UPDATE vpusers SET firstname = ?, lastname = ?, birthdate = ?, email = ?, gender = ?, password = ? WHERE id=?");
        echo $conn->error;
        $stmt->bind_param("ssssiss", $name, $surname, $birthDate, $gender, $email, $pwdhash);
        if($stmt->execute()){
            $notice = "Profiil edukalt uuendatud!";
        } else {
            $notice = "Profiili uuendamisel tekkis tõrge! " .$stmt->error;
