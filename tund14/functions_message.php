...
function storeMessage($message){
	...
	$stmt = $conn -> prepare("INSERT INTO vpmsg2 (userid, message) VALUES(?,?)");
	echo $conn -> error;
...
function readAllMessages(){
	$notice = null;
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn -> prepare("SELECT message, created FROM vpmsg2 WHERE deleted IS NULL ORDER BY created DESC");
	echo $conn -> error;
	$stmt -> bind_result($messageFromDb, $createdFromDb);
	$stmt -> execute();
	while($stmt -> fetch()){
